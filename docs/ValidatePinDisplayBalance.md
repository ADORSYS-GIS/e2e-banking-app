...
The system is made up of three core entities which interact for its implementation.<br>
- SERVICE
- CONTROLLER
- REPOSITORY

**CONTROLLER**
>The controller layer controls the routing and mapping of urls. It also makes calls to the SERVICE and obtain the values from it.
...
#
[Note](): It should be noted that these layers are actually implemented as java Packages.
...
## How they resolve the Implementation issues raised above
 ...
 in the **SERVICE** layer of our application and will serve to generate a ```procedureID``` that will uniquely identify this new procedure engaged by the user.
   - After the ```procedureID``` is generated, a ```storeprocedureID(GeneratedID)``` function implemented at the **REPOSITORY** layer takes this ```procedureID``` along with the **Phone Number** of the user and stores them as a key-value pair in the database.
...
classDiagram
    ...
    class Service {
        ...
        + checkprocedureID(ID*, ID) boolean
        + checkpin(PIN, PIN*) boolean
        + getpin(Phonenumber) PIN
        + getprocedureID() long
        - storeprocedureID(PhoneNumber, ID) long
        + handleError()
 }
